...
function OneOfStations({ title, navigation, setNavigation }) {
  ...
  useEffect(() => {
    ...
    fetchIt_1(
      `fuelIn/pagi/by-date/1?sDate=${fuelInStartDate}&eDate=${fuelInEndDate}&stationId=${station}&accessDb=${user.accessDb}`,
      user.token
    );

    fetchIt_2(
      `/fuel-balance/pagi/1?sDate=${isoStartDate}&stationId=${station}&accessDb=${user.accessDb}`,
      user.token
    );
    fetchIt(
      `/detail-sale/statement-report?sDate=${fuelPumpStartDate}&eDate=${fuelPumpEndDate}&stationDetailId=${station}&accessDb=${user.accessDb}`,
      user.token
    );
    fetchIt_3(
      `/detail-sale/by-date/?sDate=${start}&eDate=${end}&stationDetailId=${station}&accessDb=${user.accessDb}`,
      user.token
    );
  ...
  const handleFuelInSearch = () => {
    fetchIt_1(
      `fuelIn/pagi/by-date/1?sDate=${fuelInStartDate}&eDate=${fuelInEndDate}&stationId=${stationId}`,
      user.token
    );
  };
  ...
  const handleFuelBalanceSearch = () => {
    let isoStartDate = fuelBalanceStartDate.toLocaleDateString("fr-CA");
    fetchIt_2(
      `/fuel-balance/pagi/1?sDate=${isoStartDate}&stationId=${stationId}`,
      user.token
    );
  };
  ...
  const handlePumpMeterSearch = () => {
    fetchIt(
      `/detail-sale/statement-report?sDate=${fuelPumpStartDate}&eDate=${fuelPumpEndDate}&stationDetailId=${stationId}`,
      user.token
    );
  };
  ...
  return (
    <>
      <div className="w-[100%] pb-[100px] relative">
        {loading ? (
          <div className=" h-[90svh] w-[100%] bg-[#e0dfdfac] absolute top-0 z-50 left-0 right-0 bottom-0">
          ...
          <>
            <HeadCap
              station={language.stations}
              setNavigation={setNavigation}
              title={language.station_over_view}
            />
            ...
            {/* <div
              ...
              className="bg-blue-900 text-white mt-2 ml-5 p-2 flex items-center cursor-pointer hover:gap-4 hover:rounded-lg duration-700 gap-3 justify-center  text-lg "
            >
              {language.to_table}
              <AiOutlineArrowRight />
            </div> */}
            ...
            {/* nozzles */}
            {language.stations === "စတေရှင်" ? (
              <div className="flex  mt-[40px] justify-center items-center px-14">
                <h3 className=" text-xl text-left  w-[97%] mx-auto font-extrabold uppercase text-[#333]">
                  {stationName} {language.the_pump_meter}
                </h3>{" "}
                <button
                  onClick={() => setNavigation(3)}
                  class="cssbuttons-io-button2"
                >
                  {language.to_table}
                  <div class="icon">
                    <svg
              ...
              <h3 className="text-left text-xl mt-[40px]  w-[97%] mx-auto font-extrabold uppercase text-[#333]">
                {language.the_pump_meter}
                {stationName}
              </h3>
            )}
            ...
            <div className="w-[97%] mb-[40px]  drop-shadow-md  gap-5 mt-[30px]  flex justify-around items-center mx-auto relative">
              ...
              />
              <button
                ref={pumpSearchButton}
                onClick={handlePumpMeterSearch}
                className="border-2 mt-8 text-white  rounded-md px-4 p-2 bg-blue-800 hover:bg-blue-700 flex  items-center justify-center gap-2"
              >
                {" "}
            ...
            <div
              className={`w-[97%] bg-white drop-shadow-md py-5 px-3  overflow-hidden gap-5  flex flex-wrap justify-center items-center mx-auto relative mt-5 ${
                moreNozzle ? "" : "h-[280px]"
              }`}
            >
              ...
              {moreNozzle ? (
                <p
                  onClick={handleClick}
                  className="absolute bottom-0 cursor-pointer px-4 t ext-[13px] py-1  bg-blue-900 flex items-center group justify-center text-[white] gap-3 "
                >
                  {language.less_nozzle_meter}
                  <MdExpandLess className="scale-[1.5] duration-500 group-hover:rotate-180" />
                </p>
              ) : (
                <p
                  onClick={handleClick}
                  className="absolute bottom-0 cursor-pointer px-4 t ext-[13px] py-1 rounded-t-md bg-blue-900 flex items-center justify-center text-[white] gap-3 group"
                >
                  {language.more_nozzle_meter}
                  <MdExpandLess className="scale-[1.5] group-hover:rotate-180 duration-700" />
                </p>
              )}
            </div>
            {/* nozzles */}
            ...
            {/* fuelin */}
            {language.stations === "စတေရှင်" ? (
              <h3 className="text-left w-[97%] mx-auto mb-[40px] mt-[60px] border-dashed font-extrabold text-xl  uppercase text-[#333]">
                {stationName} {language.the_fuel_in_of_kyawsan}
              </h3>
            ) : (
              <h3 className="text-left  w-[97%] mx-auto mb-[40px] mt-[60px] border-dashed font-extrabold  text-xl  uppercase text-[#333]">
                {language.the_fuel_in_of_kyawsan}
                {title}
              </h3>
            )}
            ...
            {/* Tank balance */}
            {language.stations === "စတေရှင်" ? (
              <h3 className="text-left  w-[97%] mx-auto mb-5 mt-[60px] font-extrabold uppercase text-[#333] text-xl">
                {stationName} {language.the_fuel_balance_}
              </h3>
            ) : (
              <h3 className="text-left  w-[97%] mx-auto mb-5 mt-[60px] font-extrabold uppercase text-[#333] text-xl">
                {language.the_fuel_balance_} {stationName}
              </h3>
            )}
            <div className="w-[97%] mb-[40px]  drop-shadow-md  gap-5 mt-4 flex justify-around items-center mx-auto relative">
              <CalenderComponent
              ...
              />
              <button
                ref={fuelBalanceButton}
                onClick={handleFuelBalanceSearch}
                className="border-2 mt-8 text-white  rounded-md px-4 p-2 bg-blue-800 hover:bg-blue-700 flex  items-center justify-center gap-2"
              >
                {" "}
                <AiOutlineSearch size={20} />
            ...
            <div className="w-[97%] flex items-center justify-center gap-3 flex-wrap mt-[20px] drop-shadow-md  mx-auto   p-[10px]">
              ...
              {fuelBalanceData.length === 0 ? (
                <h3 className="text-center mb-5 mt-[40px] font-extrabold uppercase text-[#333] text-[25px]">
                  {title} {language.the_fuel_balance_}
                </h3>
              ) : (
