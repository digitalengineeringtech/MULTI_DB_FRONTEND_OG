...
function InputPage() {
  ...
  useEffect(() => {
    ...
    instance
      .get(`/fuelIn/pagi/1?stationId=${stationId}`, {
        headers: {
  ...
  return (
    <div className=" w-[calc(100%-70px)] relative  dashboard-right mx-auto h-[88svh] overflow-y-scroll p-[20px]">
      <div className="bg-white p-5 h-[80svh] drop-shadow-2xl ">
        ...
        <form
          onSubmit={handleAdd}
          className="mt-[30px]  flex gap-5 flex-wrap items-center"
        >
          <div className="flex flex-col gap-2">
            <label htmlFor="username">Receive Liters</label>
            <InputText
              value={receiveLiters}
              onChange={(e) => setReceiveLiters(e.target.value)}
              required={true}
          ...
          <div className="w-[100%]">
            <button
              type="submit"
              className=" hover:scale-[1.05] duration-300 border-[0.5px] hover:bg-black hover:text-white border-black mt-2 w-[100px] p-2 flex items-center justify-around rounded-md"
            >
              {" "}
              <RiAddCircleFill size={20} /> Add
            </button>
          </div>
        </form>
        {severErrorMsg ? (
          <p className=" container mx-auto uppercase text-red-400 mt-[30px]">
            {severErrorMsg}
          </p>
        ) : (
          ""
